...
## [OPEN] Configure-spawned second task page-faults on its first instruction, AddressSpace slot9 is not the same address space its code was mapped into
...
Fix: not yet applied. Two candidates, pending a decision: (a) stop retyping a fresh AddressSpace for slot9 in the self-test, and instead grant/copy/mint a capability referencing `user_as`'s own raw id into slot9, so Configure genuinely runs the second task in the same address space its code was mapped into, matching what the comment already assumed was happening; or (b) actually map `CODE2_VADDR`/`STACK2_VADDR` into the address space retype produces, which needs a way to turn the raw `KernelObjectId` retype() returns back into a usable `AddressSpace` handle (`paging::AddressSpace::from_raw()`, previously deferred as dead code, see the 2026-08-01 Untyped/Retype changelog entry).
Commit/ref: kernel: PCI enumeration + verify-before-commit design principle (exposed by, not caused by, this feature)
