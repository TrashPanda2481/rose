...
- Frame caps minted here are usable now: the Frame-Map syscall (see its own section below) maps a retyped Frame into an AddressSpace. Unmap still isn't implemented.
...
- A retyped AddressSpace is real and immediately usable by `paging.rs`. The Frame-Map syscall (see its own section below) is what actually puts something into it.

## Frame-Map

Retype hands out Frame and AddressSpace capabilities but leaves both unused. Frame-Map is the syscall that actually maps a Frame into an AddressSpace.

Implemented (`kernel/src/syscall.rs`, `kernel/src/idt.rs`, `kernel/src/usermode.rs`): `SYS_FRAME_MAP` (0x1006) takes a Frame cptr, an AddressSpace cptr, a target virtual address, and a flags word (`MAP_FLAG_WRITABLE`, `MAP_FLAG_NO_EXECUTE`). Dispatch checks the Frame cap has MAP rights (and WRITE too if the writable flag is set), checks the AddressSpace cap has MAP rights, checks the virtual address is frame-aligned, then calls `AddressSpace::map_page` on the target. Own error space (`MapError`: InvalidFrame, InvalidAddressSpace, Misaligned, codes 1-3, encoded with a +30 offset so they don't collide with CSpaceError, UntypedError, or ConfigureError).

This closes a real bug, not just a missing feature. The usermode self-test's second task had its code and stack mapped into the kernel's boot-time `user_as`, not into the separate AddressSpace Configure actually runs the task in. On a real hardware timer preemption (Oracle VM VirtualBox), the task page-faulted on its first instruction. The ring-3 self-test now issues two Map syscalls before Configure runs: one maps the already-retyped-but-unused Frame from the Retype self-test into the target AddressSpace for the stack, one maps a separately pre-populated Frame holding the second task's real code into the same AddressSpace. No growth to the Untyped pool was needed; the stack page reuses a Frame that Retype had already produced. See `BUGS.md` for the full bug writeup and `CHANGELOG.md` for the implementation notes.

Verified on BIOS+UEFI QEMU, zero build warnings, all self-test steps pass including both new Map steps, no page fault. Confirmed in Oracle VM VirtualBox, where the original crash was seen. Promoted to `stable`.
...
Update: increment 1 implemented (`kernel/src/endpoint.rs`, `kernel/src/syscall.rs`, `kernel/src/idt.rs`): Endpoint object plus blocking Send/Receive only, no Call yet. This increment's `Message{label: u32, data0: u64, data1: u64}` is a two-word subset of the fuller struct above (no length/cap_count/data[]/caps[]); the smaller shape is what a self-test can exercise end to end now, not a change to the spec above. `SYS_ENDPOINT_SEND` (0x1007) / `SYS_ENDPOINT_RECEIVE` (0x1008). No badge yet, since nothing mints Endpoint caps with distinct badges yet either. Verified on QEMU BIOS and UEFI: Receive-side delivery confirmed (the usermode self-test's second task receives the first task's blocked Send message, label/data0/data1 match). Send-side resume (the first task waking back up after `scheduler::wake()`, and the self-test's own final aggregate report) depends on a real timer preemption to complete the round-trip; this sandbox's QEMU/TCG never delivers real hardware IRQ0 (see `TROUBLESHOOTING.md`), so that line can't be observed here, the same pre-existing limitation already noted elsewhere in this doc and in `BUGS.md`, not a new issue. Confirmed in Oracle VM VirtualBox: real hardware timer ticks (`50 ticks elapsed via hardware irq0, 510ms since boot`) let the first task resume normally after `wake()`; full serial log shows the Send self-test step passing and ends with `usermode self-test: full boot sequence confirmed`. Round-trip works as designed once real preemption is available; the QEMU/TCG gap above was purely an artifact of this dev sandbox, not a design or implementation issue.
...
Update: the gap above (single global `TSS.RSP0`, never updated per-task) is fixed. Each `Task` now carries its own `kernel_stack_top`, and `tick()`/`yield_now()` reload `TSS.RSP0` from it on every switch-in, unconditionally, right alongside the existing CR3 reload. See the Frame-Map section above and `BUGS.md`/`CHANGELOG.md` for the full writeup; this is what let two ring-3 tasks actually coexist under real hardware preemption without corrupting each other's trap state.
