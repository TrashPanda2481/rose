...
Implemented (`kernel/src/cspace.rs`): fixed 64-slot table, CPtr 0 reserved as the null cap. No syscall reaches it yet and no `Task` owns one yet; only a boot self-test exercises it directly. That wiring is the next feature.
