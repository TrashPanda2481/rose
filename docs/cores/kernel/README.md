...
Separately: the scheduler's `context_switch`/`Task` struct has no notion of address spaces yet; task switches never touch CR3. Fine today since every scheduler task runs cooperatively inside the kernel's own AddressSpace. Needs a CR3 reload wired into `context_switch` before the scheduler can run multiple components each in their own AddressSpace.
