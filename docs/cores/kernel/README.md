...
Update: Untyped/Retype landed (see "Untyped/Retype" section below). It still isn't a generic object heap for every type above, just Frame and CSpace in v0.1; a Frame's `KernelObjectId` is its physical address (same "reuse a stable identity" pattern as AddressSpace), a CSpace's is a registry index into a new module-local table (`kernel/src/untyped.rs`'s `CSPACE_OBJECTS`), since a freshly retyped CSpace has no other stable address to reuse.

## Untyped/Retype

Untyped is a pool of individually-allocated physical frames, not yet shaped into anything. Retype pops one frame out of that pool and turns it into a Frame or CSpace object; those are the only two retypeable types in v0.1, everything else (PageTable, AddressSpace, Thread, Endpoint, Notification, Reply, IrqHandler) is a future feature added to `retype`'s match arm when its turn comes.

Implemented (`kernel/src/untyped.rs`, `kernel/src/syscall.rs`): `UntypedObject::new(count)` pulls `count` frames from the global frame allocator up front; `retype(untyped_id, object_type)` takes the next frame off that pool's watermark and, depending on `object_type`, either hands back the frame's own physical address (Frame) or boxes a new `CSpace` into a module-local registry and hands back its index (CSpace). Reached via `SYS_UNTYPED_RETYPE` (0x1004): args are the Untyped cptr, the raw `ObjectType` byte (see `abi`'s `ObjectType::to_u8`/`from_u8`), and the destination cptr for the newly minted cap. Dispatch checks everything it can before calling `retype` (both cptrs resolve, the source cap really is Untyped, `object_type` is a real and retypeable variant, the destination slot is empty) since a frame `retype` hands out has no way back into the pool if the install after it is rejected; `CSpace::install_child` (new) is what performs that install, recording the new cap as a child of the Untyped cap in the same derivation tree `copy`/`mint` already use, so revoking the Untyped cascades into whatever was retyped from it.
...
- Only Frame and CSpace are retypeable.
- A retyped CSpace is bookkept as one frame charged out of the pool, but the actual `CSpace` struct still lives on the kernel heap via `Box`, not placed in that physical frame. Documented asymmetry.
