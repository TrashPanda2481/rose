...
## PIT/PIC hardware timer (IRQ0) never reaches the CPU in this sandbox's QEMU/TCG
Context: bringing up the PIC remap + PIT timer (`kernel/src/pic.rs`, `kernel/src/timer.rs`), first boot test.
Symptom: kernel programs the PIC and PIT correctly, executes `sti`, but the timer tick handler never runs; no crash, just silence, forever.
Root cause: isolated to this dev sandbox's QEMU 10.2.1 + TCG software emulation specifically. Byte-verified via objdump that PIC remap (ICW1-4, IRQ0-7 to vectors 32-39) and PIT programming (command 0x36, divisor 11931 for 100Hz) are correct; runtime probes confirmed RFLAGS.IF=1 and PIC IMR=0xfe. PIC IRR read back as 0x01 (IRQ0 pending) and stayed pinned there; the CPU never took it. A software-triggered `int 0x20` from the same code path dispatched instantly, proving the IDT stub, dispatch, EOI, and tick counter are all correct. QEMU's own `-d int` trace showed zero interrupt-servicing events during the whole window. Reproduced identically on BIOS and UEFI, and with `-accel tcg` explicit. Confirmed as a sandbox-only issue (not a kernel bug) by booting the same ISO in Oracle VM VirtualBox, which uses real hardware virtualization instead of TCG: the tick handler fired immediately there and the counter advanced normally.
Resolution: not fixable on our end, it's outside the kernel. `timer_selftest` bounds its wait with an RDTSC-based timeout instead of spinning forever, and falls back to a software `int 0x20` so boot still completes cleanly in this sandbox. Use VirtualBox (or real hardware) for any test that actually needs to observe live hardware timer ticks; this sandbox's QEMU can be used for everything else.
